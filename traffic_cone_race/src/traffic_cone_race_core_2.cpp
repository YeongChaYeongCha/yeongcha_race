...
void traffic_cone_race::cone_callback(const visualization_msgs::MarkerArray &cone_boundingbox){
  ...
  if(cone_boundingbox.markers.size()>=3){
    ...
    for (auto iter=closest_traffic_cone_.begin(); iter<=closest_traffic_cone_.end(); iter++) {
      if (iter->second > 0.0) { 
        float distance = sqrt(pow(iter->first, 2) + pow(iter->second, 2));
        if (distance < sqrt(pow(nearestPositivePoint.first, 2) + pow(nearestPositivePoint.second, 2))) {
          nearestPositivePoint = *iter;
        }
      }
      else if (iter->second < 0.0) { 
        float distance = sqrt(pow(iter->first, 2) + pow(iter->second, 2));
        if (distance < sqrt(pow(nearestNegativePoint.first, 2) + pow(nearestNegativePoint.second, 2))) {
          nearestNegativePoint = *iter;
        }
      }
    }
    sort(closest_traffic_cone_.begin(), closest_traffic_cone_.end(), cmp);
    remove(closest_traffic_cone_.begin(), closest_traffic_cone_.end(), nearestPositivePoint);
    remove(closest_traffic_cone_.begin(), closest_traffic_cone_.end(), nearestNegativePoint);
    ...
    avg_x = (nearestPositivePoint.first + nearestNegativePoint.first + closest_traffic_cone_[0].first) / 3;
    ...
    if(dis!=0){
      float ldp_deg=-RAD2DEG(atan(way_point_[0].second/way_point_[0].first));
      float steer=RAD2DEG(atan(way_point_[0].second/way_point_[0].first))*drive_ratio_;
      // float steer=-RAD2DEG(atan((2*sin(ldp_deg)*1.04)/sqrt(pow(way_point_[0].first,2)+pow(way_point_[0].second, 2))));
      // float steer=RAD2DEG(atan(way_point_[0].second/way_point_[0].first))*drive_ratio_;
