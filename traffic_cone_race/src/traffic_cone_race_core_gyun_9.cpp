...
traffic_cone_race::traffic_cone_race(ros::NodeHandle &nh){
  cone_left_pos_=nh.subscribe("/left_cone_box", 1000, &traffic_cone_race::left_cone_callback, this);
  cone_right_pos_=nh.subscribe("/right_cone_box", 1000, &traffic_cone_race::right_cone_callback, this);
...
void traffic_cone_race::process_data(){
  ...
  // if(left_traffic_cone_.size()>0 || right_traffic_cone_.size()>0){
    ...
    if(sqrt(pow(avg_x,2)+pow(avg_y,2))<0.5){
    //   cout<<"Search Way Point Again"<<"\n";
    //   negativePoint=*(left_traffic_cone_.begin()+1);
    //   positivePoint=*(right_traffic_cone_.begin()+1);
    //   cout<<"------------------"<<"\n";
  ...
  steer = -RAD2DEG(atan((2 * sin(ldp_deg) * 1.04) / sqrt(pow(way_point_[0].first - 0.43 , 2) + pow(way_point_[0].second, 2))));
