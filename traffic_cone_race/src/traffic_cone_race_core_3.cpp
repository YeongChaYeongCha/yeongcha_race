...
void traffic_cone_race::cone_callback(const visualization_msgs::MarkerArray &cone_boundingbox){
  ...
  if(cone_boundingbox.markers.size()>=3){
    ...
    for (auto iter=closest_traffic_cone_.begin(); iter!=closest_traffic_cone_.end(); iter++) {
      if (iter->second > 0.0) { 
        float y_pos=iter->second;
        if (y_pos>positivePoint.second) {
          positivePoint = *iter;
        }
      }
      else if (iter->second < 0.0) { 
        float y_pos=iter->second;
        if (y_pos<negativePoint.second) {
          negativePoint = *iter;
        }
      }
    }
    ...
    if(dis!=0){
      ...
      float steer = -RAD2DEG(atan((2 * sin(ldp_deg) * 1.04) / sqrt(pow(way_point_[0].first - 0.97, 2) + pow(way_point_[0].second, 2))));
