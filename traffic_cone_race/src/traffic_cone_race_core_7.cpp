...
void traffic_cone_race::cone_callback(const visualization_msgs::MarkerArray &cone_boundingbox){
  ...
  if(cone_boundingbox.markers.size()>=3){
    ...
    if((sqrt(pow((closest_traffic_cone_.begin())->first,2)+pow(closest_traffic_cone_.begin()->second,2))>sqrt(pow((closest_traffic_cone_.begin()+1)->first,2)+pow((closest_traffic_cone_.begin()+1)->second,2))) && closest_traffic_cone_.size()>=4){
    ...
    else if((sqrt(pow((closest_traffic_cone_.end()-1)->first,2)+pow((closest_traffic_cone_.end()-1)->second,2))>sqrt(pow((closest_traffic_cone_.end()-2)->first,2)+pow((closest_traffic_cone_.end()-2)->second,2))) && closest_traffic_cone_.size()>=4){
      ...
      thirdPoint=(sqrt(pow(closest_traffic_cone_.begin()->first,2)+pow(closest_traffic_cone_.begin()->second,2))>sqrt(pow((closest_traffic_cone_.end()-1)->first,2)+pow((closest_traffic_cone_.end()-1)->second,2))) ? *(closest_traffic_cone_.begin()) : *(closest_traffic_cone_.end()-1);
    }
    ...
    float steer = -RAD2DEG(atan((2 * sin(ldp_deg) * 1.04) / sqrt(pow(way_point_[0].first - 0.45 , 2) + pow(way_point_[0].second, 2))));
    ...
    if(way_point_[0].first>0.2){
      ...
      way_point_angle.ns="cylinder";
      way_point_angle.id=0;
      way_point_angle.type=3;
      way_point_angle.action=0;
      way_point_angle.pose.position.x=0.0;
      way_point_angle.pose.position.y=0.0;
      way_point_angle.pose.position.z=0.0;
      way_point_angle.pose.orientation.x=-way_point_[0].first;
      way_point_angle.pose.orientation.y=way_point_[0].second;
      way_point_angle.pose.orientation.z=0.0;
      way_point_angle.pose.orientation.w=1.0;
      way_point_angle.scale.x=0.1;
      way_point_angle.scale.y=0.1;
      way_point_angle.scale.z=1.0;
      way_point_angle.color.a=1.0;
      way_point_angle.color.r=1.0;
      way_point_angle.color.g=0.0;
