...
def plot_waypoints(points):
    points_array = np.array(points, dtype=np.float64)
    tri = Delaunay(points_array)
    waypoints = []

    for simplex in tri.simplices:
        p1, p2, p3 = points_array[simplex]
        centroid = (p1 + p2 + p3) / 3
...
def BoundingBox_callback(msg):
	...
	intersections=plot_waypoints(traffic_cone_pose)
...
while not rospy.is_shutdown():
	...
	waypoints = plot_waypoints(traffic_cone_pose)


	for i, intersection in enumerate(waypoints):
